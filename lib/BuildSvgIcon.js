...
module.exports = {
    ...
    for (const folder of File.readFolder(this._toDir)) {
      html += `<section class="block e032ojhc">
        ...
        <div class="block e0ht91h5 preview-icon-cells">
          ${this.getPreviewCells(folder.name, folder.children)}
        </div>
      </div>`
    }
    return html
  ...
  getPreviewCells (folder, files) {
    let html = ''
    for (const file of files) {
      const component = this.getComponentData(folder, file.name)
      html += `<div class="block e0yhav44 preview-icon-cell">
        <div class="component" data-ss-component="${component}"></div>
  ...
  getComponentData (folder, file) {
    const data = {
      file: `component/icon/${folder}/${file}`,
      ref: Crypto.generateElementRef()
    }
